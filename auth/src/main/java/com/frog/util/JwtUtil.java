...
public class JwtUtil {
    ...
    private boolean validateTokenFingerprint(String jti, UUID userId) {
        ...
        Object raw = fingerprint.get("userId");
        if (raw instanceof UUID uuid) {
            storedUserId = uuid;
        } else if (raw instanceof String uuid) {
            storedUserId = UUID.fromString(uuid);
        } else {
            throw new IllegalArgumentException("userId is not a UUID or String");
        }
